...
//dep : service layer i/f
	...
	/*
	 * Emp authentication (signin) Request Body : DTO (eg : AuthRequestDTO) Resp :
	 * ResponseEntity Success : SC 200 + AuthRespDTO Failure : SC 404 + ApiResp :
	 * err mesg +timestamp
	 */
	...
	public ResponseEntity<?> authenticateEmp(@RequestBody @Valid  AuthRequestDTO request) {
		{
			System.out.println("in emp auth " + request);
			try {
			return new ResponseEntity<>
			(empService.authenticateEmp(request), HttpStatus.OK);
