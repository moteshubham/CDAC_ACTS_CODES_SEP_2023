...
public class CustomerManagement {
	...
	public static void main(String[] args) {
		try (Scanner sc = new Scanner(System.in);
				PrintWriter pw = new PrintWriter(new FileWriter("just_witten.txt"));
				BufferedReader br = new BufferedReader(new FileReader("just_witten.txt"))) {
...
//						Collections.sort(listCust);
					...
					case 8:
						ArrayList<Customer> arrcust = new ArrayList<>(customers.values());
						for(Customer c : arrcust) {
							pw.write(c+"\n");
						}
						pw.close();
					System.out.println("Written to just written file");
						break;
						
					case 9:
						System.out.println("Reading from just written file");
					br.lines()
					.map(s->s.toUpperCase())
